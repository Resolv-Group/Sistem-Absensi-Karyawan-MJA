...
@section('content')
    ...
    <div class="bg-white rounded-[2.5rem] border border-gray-200 p-5 sm:p-8 md:p-10 relative group transition-all duration-300 hover:border-blue-300 hover:shadow-[0_20px_50px_rgba(0,0,0,0.04)] space-y-10 sm:space-y-12 mb-10">

        {{-- 1. HEADER BARIS --}}
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-50 pb-6">
            <div class="flex items-center gap-4">
                <div class="h-10 w-10 bg-blue-600 text-white text-sm font-black rounded-2xl flex items-center justify-center shadow-lg shadow-blue-200 shrink-0" x-text="index + 1"></div>
                <div>
        ...
        <div class="space-y-6" x-show="window.unitInfo.sistem_pengajian != 2">
            ...
            <div class="sm:col-span-2 mt-4">
                ...
                <div class="overflow-x-auto -mx-1 px-1">
                    <div class="flex bg-slate-50 p-1.5 rounded-[1.5rem] border border-slate-100 shadow-inner min-w-[480px]">
                        <template
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div class="sm:col-span-2 mt-2" x-data="{ fileName: '' }">
                    <label class="flex items-center gap-4 p-4 sm:p-6 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[1.5rem] cursor-pointer hover:bg-white hover:border-blue-400 transition-all group shadow-sm">
                        <div class="shrink-0 h-10 w-10 sm:h-14 sm:w-14 bg-white rounded-2xl shadow-sm border border-slate-100 flex items-center justify-center text-slate-400 group-hover:text-blue-500 transition-colors">
                            <svg class="w-5 h-5 sm:w-7 sm:h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                        </div>
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 leading-relaxed" x-show="!fileName">Klik atau drag file PDF/Gambar baru ke sini</p>
                            <p class="text-[10px] font-bold text-emerald-500 uppercase tracking-widest mt-1" x-show="fileName">File siap diunggah</p>
                        </div>
                        <input type="file" class="hidden" :name="`pekerja[${index}][dokumen_pkwt]`" accept=".pdf,.jpg,.jpeg,.png" @change="fileName = sanitizeFileInput($event.target)">
                    </label>
                </div>
            </div>
        </div>
