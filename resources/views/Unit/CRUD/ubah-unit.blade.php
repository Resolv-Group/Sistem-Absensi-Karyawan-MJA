    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Ubah Unit</h1>
                    <p class="text-sm text-gray-500 mt-1">Ubah formulir di bawah untuk memperbarui unit.</p>
                </div>
            </div>
        </div>
        ...
        {{-- FORM CARD --}}
        <form id="formUbahUnit" action="{{ route('update.unit', $unit->id) }}" method="POST" enctype="multipart/form-data"
            x-ref="updateForm" x-data="updateFormHandler()" @submit.prevent="confirmSubmit"
                ...
                <div x-data="tunjanganManager(@js($unit->tunjangan))">
                    ...
                    <div
                        ...
                        <div class="md:col-span-7">
                            <label
                                class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Tambah
                                Kategori Baru</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="relative group">
                                    ...
                                    <input type="number" name="bpjs_naker" placeholder="0" min="0"
                                        max="100" step="0.01" value="{{ old('bpjs_naker', $unit->bpjs_naker) }}"
                                        class="block w-full rounded-xl border-gray-200 bg-gray-50
                                        text-gray-900 font-bold text-sm placeholder-gray-400
                                        focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10
                                        transition-all duration-200 py-3 pl-4 pr-10">
                        ...
                        <div class="md:col-span-2" x-data="{ fileName: '' }">
                            ...
                            <div class="mt-2 flex items-start gap-2 text-xs text-gray-500 italic">
                                <svg class="w-4 h-4 mt-0.5 text-gray-400 flex-shrink-0" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M13 16h-1v-4h-1m1-4h.01M12 2a10 10 0 100 20 10 10 0 000-20z" />
                                </svg>
                                <p>Kosongkan jika tidak ingin mengganti dokumen kontrak yang sudah ada.</p>
                            </div>
                        </div>
    ...
    <script>
        ...
        function picCombobox() {
            return {
                ...
                init() {
                    // 1. Old input has priority, fallback to edit data
                    let ids = @json(old('pic_ids', $selectedPicIds ?? []));
                    ...
                    // 3. Map IDs → full objects
                    this.selectedItems = this.list.filter(item =>
                        ids.includes(String(item.val))
                    );
                },
        ...
        function tunjanganManager(initialData = null) {
            ...
            if (initialData) {
                startingList = Object.keys(initialData).map(key => {
                    return {
                        nama: key.charAt(0).toUpperCase() + key.slice(1).replace(/_/g,
                        ' '), // Format agar rapi (Score, Driver)
