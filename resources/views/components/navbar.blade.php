<nav x-data="{ mobileMenuOpen: false }"
    ...
    {{-- Role Based Menus --}}
    @foreach ([
        ['role' => ['admin', 'hrd', 'head_supervisor', 'pic'], 'route' => 'view.pekerja', 'label' => 'Pekerja', 'pattern' => ['view.pekerja', 'view.tambah.pekerja', 'view.detail.pekerja', 'view.ubah.pekerja'], 'icon' => 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z'],
        ['role' => ['admin', 'hrd', 'akuntan', 'pic'], 'url' => '/payroll', 'label' => 'Payroll', 'pattern' => 'payroll*', 'icon' => 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.407 2.646 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.407-2.646-1M12 16a6.002 6.002 0 006-4m-12 0a6.002 6.002 0 006 4m6-4V7a1 1 0 00-1-1H7a1 1 0 00-1 1v10a1 1 0 001 1h10a1 1 0 001-1v-4'],
        ['role' => ['admin', 'hrd', 'akuntan', 'head_supervisor'], 'route' => 'view.staff', 'label' => 'Staff', 'pattern' => ['view.staff', 'view.tambah.staff', 'view.detail.staff', 'view.ubah.staff'], 'icon' => 'M21 13.255A2.396 2.396 0 0019.5 13H17c-1.105 0-2 .895-2 2s.895 2 2 2h2.5c.39 0 .753-.105 1.055-.255A5.002 5.002 0 1121 13.255zM11 13.255A2.396 2.396 0 009.5 13H7c-1.105 0-2 .895-2 2s.895 2 2 2h2.5c.39 0 .753-.105 1.055-.255A5.002 5.002 0 1111 13.255z'],
        ['role' => ['akuntan'], 'url' => '/kas-kecil-asset', 'label' => 'Kas Kecil & Asset', 'pattern' => 'kas-kecil-asset*', 'icon' => 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10'],
        ...
        @php
            ...
            $isActive = isset($item['route']) ? request()->routeIs(...$patterns) : Request::is(...$patterns);
        @endphp
    ...
    @endforeach
        ...
        {{-- Penilaian Logic --}}
        @if(Auth::user()->role === 'pic' && Auth::user()->staff && Auth::user()->staff->units->count())
            <li class="relative group">
                <button
                    class="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all duration-300
                    ...
                    <span>Penilaian ({{ Auth::user()->staff->units->count() }})</span>
