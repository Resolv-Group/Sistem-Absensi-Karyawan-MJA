<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
        <div>
            ...
            <p class="text-base font-semibold text-gray-900">{{ $staff->emergency_nama ?? '-' }}</p>
        </div>
        <div>
            <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Hubungan</label>
            <p class="text-base font-semibold text-gray-900">{{ $staff->emergency_hubungan ?? '-' }}</p>
        </div>
        <div>
            <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Nomor Telepon</label>
            <p class="text-base font-semibold text-gray-900">{{ $staff->emergency_telp ?? '-' }}</p>
        </div>
        <div>
            <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Nama Ibu Kandung</label>
            <p class="text-base font-semibold text-gray-900">{{ $staff->ibu_kandung ?? '-' }}</p>
        </div>
    </div>
</div>
